...
## Overview
Polish-language Streamlit dashboard for ETF analysis using Gary Antonacci's Global Equity Momentum (GEM) strategy. Analyzes 37 ETFs globally + 140 Polish stocks from GPW (WIG20/mWIG40/sWIG80) + ~200 cryptocurrencies + ~456 US stocks from S&P 500. 25 pages total (0–24; 4 free, 21 premium).
...
data/
  ...
  gpw_universe.py       # 140 Polish stocks (WIG20:20 + mWIG40:40 + sWIG80:80) with .WA suffix
...
components/
  ...
  charts.py             # Plotly charts (price, momentum, drawdown, sparkline, seasonality heatmap, fan chart, TA overview/MACD/RSI, picks return bar)
  cards.py              # Signal cards, metric cards, macro cards, stats table, TA signal card, data status, alert banner, top pick cards + inline SVG sparkline, section band, kpi card (st.html)
...
pages/
  ...
  7_sp500.py            # S&P 500 US stocks (8 tabs: ranking, wykresy, porownanie+stats, backtest, RS, screening, finanse, sprawozdania Q)
  8_gpw.py              # Polish stocks GPW (8 tabs: ranking, wykresy, porownanie+stats incl. WIG20/mWIG40/sWIG80, backtest, RS, screening, finanse, sprawozdania Q)
  9_top_picks.py        # Top Picks — miesieczna piatka SP500 + GPW (karty + sparkline, bar zwrotow, donut sektorow, track record)
  10_kryptowaluty.py    # Crypto (7 tabs: ranking, alt/BTC ratio, wykresy, porownanie+stats, backtest, sygnaly TA, RS)
  11_portfolio.py       # Portfolio Builder (ticker selection, weights, backtest, correlation)
  12_screener.py        # Cross-Asset Screener (unified ranking from all 4 universes)
  13_makro.py           # Macro Dashboard (VIX, yield curve, DXY, S&P+200MA, gold, oil)
  14_sezonowosc.py      # Seasonality (monthly returns heatmap, avg month, Sell in May)
  15_waluta.py          # Currency PLN/USD impact on ETF returns
  16_drawdown.py        # Drawdown analysis (top N, duration, recovery, histogram)
  17_sygnaly.py         # GEM signal history (timeline, stats, calendar heatmap)
  18_monte_carlo.py     # Monte Carlo simulation (fan chart, distribution, probabilities)
  19_sektory.py         # Sector rotation (11 GICS sectors, heatmap, cycle)
  20_intermarket.py     # Intermarket analysis (overlay, ratio, rolling corr, regime)
  21_side_by_side.py    # Side-by-Side comparison of two assets (price, drawdown, RS)
  22_gem_extended.py    # GEM Extended (configurable universe + top N + safe asset fallback)
  23_earnings_calendar.py # Earnings Calendar (±28 dni), watchlist localStorage, cross-link do F13 Sprawozdania Q
  24_insider_screener.py  # Bulk Insider Screener SP500 (456), top buyers/sellers, sektorowa heatmapa, cross-link do F13 Insiders
```

**Numeracja stron = kolejnosc w sidebarze.** Top Picks siedzi pod GPW (2026-08-07), wiec
strony 9–23 przesunely sie o +1. Numer w nazwie pliku, `require_premium(N)` i klucz w
`PAGE_INFO` musza sie zgadzac; URL strony bierze sie z nazwy BEZ numeru, wiec przenumerowanie
nie psuje linkow.
...
- **Period "Strategia 12-1":** Available in Wykresy (page 3), Porownanie (page 4), Alt/BTC ratio (page 10), and comparison tabs in S&P 500 (page 7), GPW (page 8), Kryptowaluty (page 10) — downloads 2y data, trims to `iloc[-273:-21]` (the exact 12-1 momentum window)
...
- **Seasonality (page 14):** `monthly_returns_matrix()` in `momentum.py` — pivot year×month. `seasonality_heatmap()` in `charts.py` — RED→BG→GREEN. Tabs: heatmap, avg month bar, Sell in May (May-Oct vs Nov-Apr).
- **Currency PLN/USD (page 15):** `USDPLN=X` from yfinance. Formula: `ret_PLN = (1+ret_USD)*(1+ret_USDPLN)-1`. Tabs: kurs, USD vs PLN table, dual-axis overlay, hedging explainer.
- **Drawdown Analysis (page 16):** `top_drawdowns()` in `momentum.py` — iterates drawdown series, finds peak/trough/recovery. Tabs: annotated underwater chart, table, multi-asset comparison, histogram.
- **Signal History (page 17):** Reuses `backtest_gem()` with period="max" for full signal log. Tabs: current signal card, Gantt-style timeline, pie chart of time allocation, year×month calendar heatmap (colored by signal).
- **Monte Carlo (page 18):** `monte_carlo_simulation()` in `momentum.py` — bootstrap daily returns. `fan_chart()` in `charts.py` — percentile bands P5/P25/P50/P75/P95. Tabs: fan chart, final value histogram, probability cards.
- **Sector Rotation (page 19):** 11 SPDR sector ETFs (XLK,XLF,XLE,XLV,XLI,XLY,XLP,XLC,XLRE,XLU,XLB). Added 6 missing ETFs to `etf_universe.py`. Tabs: rolling 3M momentum heatmap, ranking bar, business cycle explainer, comparison.
- **Intermarket (page 20):** SPY,^TNX,GC=F,DX-Y.NYB,CL=F — cross-asset relationships. Tabs: normalized overlay, ratio pairs, rolling correlation, correlation matrix, risk-on/risk-off regime classifier. Note: uses ^TNX (yield) instead of AGG (price) — regime logic inverted for yields.
- **TA Signals (page 10, tab 6):** Trend-following system with 5 indicators, each scoring -1/0/+1:
...
- **Side-by-Side (F9):** Page 21 (`pages/21_side_by_side.py`) — two-column comparison of any two assets. Includes price chart, drawdown, stats table, correlation, RS chart. Premium page.
...
- **Sprawozdania Q (F13):** 8-my tab `📈 Sprawozdania Q` (po `💰 Finanse`) w pages 7_sp500 + 8_gpw. Hybrid Screener + Deep dive. **Screener bulk** (`ALL_SP500_TICKERS` = 456 / `ALL_GPW_TICKERS` = 140): `bulk_fetch_earnings_history()` w `data/financials.py` z ThreadPool 8 workers + exp backoff retry (1s/2s/4s) + `st.progress` + parquet cache 24h. Tabela: ticker/Q/EPS est/EPS act/EPS Δ%/beat_streak (ProgressColumn 0-8). Filtry: Beat status, Min EPS surprise %, Min beat streak, Top N, Sort by. 4 summary cards + CSV export. **Deep dive** (per ticker): header z ratios snapshot + 4 metric cards (last Q EPS, beat streak, fwd Q+1 EPS YoY, fwd Q+1 Revenue) + 4 sub-taby: **Beat/Miss historia** (Plotly bar 8Q hist + forward 4 punkty hatched + tabela rewizji konsensusu 7d/30d/60d/90d), **Income Statement / Balance Sheet / Cash Flow** (radio Kwartalne 8Q/Roczne 4Y, priority rows + reszta max 10, format B/M). Banki GPW: FCF ukryte (reuse `is_bank` + `GPW_BANKS`). Nowy moduł UI: `components/financials_ui.py` z `render_sprawozdania_screener()` + `render_sprawozdania_deep_dive()`. yfinance endpointy: `earnings_history` (już istniał), `quarterly_income_stmt/_balance_sheet/_cashflow`, `income_stmt/balance_sheet/cashflow` (roczne), `eps_trend`, `earnings_estimate`, `revenue_estimate`. Cache 24h przez parquet (pyarrow) w `data/cache/financials/{ticker}_{q|a}.parquet`. **Pułapka taka sama jak F12:** `render_sprawozdania_screener/_deep_dive` musi być na poziomie modułu PRZED `with tab8:` blokiem (`@st.fragment`). Spec: `docs/superpowers/specs/2026-06-02-finanse-kwartalne-design.md`. Plan: `docs/superpowers/plans/2026-06-02-finanse-kwartalne.md`. Tests: `tests/test_universe.py` (5) + `tests/test_financials.py` (10) = 15/15 passing (pytest, mock-based — yfinance lokalnie rate-limited).
- **Earnings Calendar (F14):** `pages/23_earnings_calendar.py` (page 23). Kalendarz nadchodzących i niedawnych raportów (±28 dni) dla SP500 + GPW (effective universe 577 — ETF wykluczone, no earnings). UI: filtry expander + 4 summary cards + Plotly heatmapa 8 tygodni (T-3 do T+4, dziś = czerwona ramka, hover do 5 tickerów) + `st.data_editor` z checkbox ⭐ watchlist + CSV export + cross-link selectbox+button do F13 deep dive. Data layer (`data/financials.py`): `fetch_earnings_dates(ticker)` z parquet cache 24h, `bulk_fetch_earnings_calendar(universe)` z ThreadPool 8 workers + retry, filter ETF (z `_get_etf_set()`). Watchlist: `components/watchlist.py` (localStorage przez streamlit-local-storage, klucz `gem_dashboard_watchlist_v1`, cross-feature module). Cross-link: `st.switch_page("pages/7_sp500.py")` + session_state `{market}_deep_dive_ticker` + `{market}_pending_view = "deep_dive"` (pattern z F13 hotfix `b1f8502`). Spec: `docs/superpowers/specs/2026-06-04-earnings-calendar-design.md`. Plan: `docs/superpowers/plans/2026-06-04-earnings-calendar.md`. Tests: `tests/test_calendar.py` 18/18 passing (helpers + fetch + watchlist).
- **Insider Trading & Institutional (F15):** 5. sub-tab `👥 Insiders` w F13 Sprawozdania Q deep dive (`components/financials_ui.py:render_sprawozdania_deep_dive`). 6 sekcji: Major Holders (4 metric cards % insider/institutional/count/float) + Insider Purchases summary + Insider Transactions (tabela + Plotly bar net buy/sell per miesiąc 6 mc, zielony/czerwony) + Insider Roster + Institutional Holders (tabela + Plotly donut top 10 + Reszta) + Mutual Fund Holders. 6 funkcji w `data/financials.py`: `fetch_insider_transactions` (z normalize Sale→Sell przez `_normalize_transaction_type`), `fetch_insider_purchases`, `fetch_insider_roster_holders`, `fetch_institutional_holders`, `fetch_mutualfund_holders`, `fetch_major_holders` (defensywny parser dict ALBO DataFrame). Cache `@st.cache_data(ttl=86400)` in-memory (lekkie endpointy, brak parquet). GPW fallback: wszystkie 6 None → info box z linkiem do gpw.pl/komunikaty-spolek (yfinance nie ma SEC Form 4 dla polskich spółek). Banki USA bez special handling. Spec: `docs/superpowers/specs/2026-06-05-insider-trading-design.md`. Plan: `docs/superpowers/plans/2026-06-05-insider-trading.md`. Tests: `tests/test_financials.py` +12 testów mock-based (46/46 full suite).
- **Bulk Insider Screener (F16):** `pages/24_insider_screener.py` (page 24). Bulk insider activity dla SP500 (456 spółek, ETF filtered, GPW pominięte). UI: filtry expander (sektor multiselect, status Buy/Sell/Mixed/All, Min |Net Value|, Min beat streak 0-6, Min Market Cap 5 opcji, Sort 9 metryk, Top N 10/25/50/100/250) + 4 summary cards (Spółek z danymi X/456, Net Buyers, Net Sellers, Avg Net) + 2 Plotly bar charts side-by-side (Top 10 Buyers green, Top 10 Sellers red) + tabela 11 kolumn (🎯 sentiment / Ticker / Nazwa / Sektor / Cap / Net 6mc / #Buys / #Sells / Top Buyer / Top Fund / Streak ProgressColumn 0-6) + sektorowa Plotly horizontal bar (sum net per sektor GICS) + cross-link selectbox+button do F13 deep dive Insiders sub-tab (`st.switch_page` + session_state pattern z F14 `b1f8502`) + CSV export. Data layer (`data/financials.py`): `_compute_buy_streak` + `_aggregate_insider_for_ticker` (16 pól) + `bulk_fetch_insider_screener` (ThreadPool 8, `@st.cache_data ttl=86400`, filter ETF). Reuse: F15 `fetch_insider_transactions` + `fetch_institutional_holders` z fallback chain, F11 `get_ratios_snapshot`, fallback sektor + nazwa z `SP500_SECTOR_MAP` + `SP500_NAMES`. Cold cache ~5-7 min (warm <2s). Spec: `docs/superpowers/specs/2026-06-05-bulk-insider-screener-design.md`. Plan: `docs/superpowers/plans/2026-06-05-bulk-insider-screener.md`. Tests: `tests/test_financials.py` +6 (52/52 full suite).
- **Watchlist cross-feature domknięcie (F17):** Rozszerzenie `components/watchlist.py` (z F14) na 3 screenery: F12 Screener Fundamentalny (`pages/7_sp500.py` + `pages/8_gpw.py` tab6), F13 Sprawozdania Q screener (`components/financials_ui.py:render_sprawozdania_screener`, oba market), F16 Bulk Insider Screener (`pages/24_insider_screener.py`). Wzorzec z F14: filter checkbox "⭐ Tylko watchlist" (disabled gdy pusty) + ⭐ kolumna w `st.data_editor` z `CheckboxColumn` + diff detection (`edited["⭐"] != display["⭐"]`) + `toggle_ticker(ls, ticker)` + `st.rerun()`. Klucze widget z `_v2` suffix (cache safety przy zmianie `st.dataframe` → `st.data_editor`). **F12 dodatkowy edge case:** `st.data_editor` nie obsługuje `on_select="rerun"` — zastąpiono mechanizm klik-row pre-fillijący `sp_finanse_ticker`/`gpw_finanse_ticker` osobnym selectboxem + button "💰 Pokaż w Finanse" (pattern z F13/F16 cross-link). Spec: `docs/superpowers/specs/2026-06-06-watchlist-domkniecie-design.md`. Plan: `docs/superpowers/plans/2026-06-06-watchlist-domkniecie.md`. Tests: brak nowych (pattern integracji = boilerplate Streamlit; sam watchlist testowany w `tests/test_calendar.py` z F14, 52/52 full suite).
...
- **Top Picks (F18):** `pages/9_top_picks.py` (page 9) + `data/top_picks.py` (silnik, **bez importu streamlita** — musi dzialac w GitHub Action) + `scripts/update_top_picks.py` (CLI dla CI). **Regula:** `latest_returns()` → `rank_based_score(12M 0.40 / 6M 0.30 / 3M 0.20 / 1M 0.10, anti_1m=True)` na dzien `asof`, filtr historii ≥273 sesji, filtr plynnosci (mediana 60d `close*volume`: **50M USD** dla SP500, **5M PLN** dla GPW), limit **max 2 spolki na grupe**, top 5 equal-weight. **Grupa = sektor, NIE indeks** — dla SP500 `SP500_SECTOR_MAP`, dla GPW nowa `GPW_SECTOR_MAP` (140/140, 12 sektorow, generowana z yfinance `Ticker.info["sector"]` z wydzieleniem `Banki` wg `GPW_BANKS`). Powod: indeks nie jest wymiarem ryzyka (WIG20/mWIG40 nie chroni przed 5 bankami), a filtr plynnosci zbija sWIG80 do zera → zostaja 2 grupy → sufit 4 pozycji zamiast 5. **Strona nic nie liczy** — czyta `data/top_picks_history.json` (append-only log; `append_snapshot()` NIE nadpisuje istniejacego klucza miesiaca) i `data/top_picks_sim.json` (pochodna symulacja, nadpisywana). `simulate_rule()` wola **to samo `select_picks()`** co log — swiadomie nie uzywa `backtest_rotation()`, ktora nie zna limitu sektorowego ani plynnosci. Symulacja ma survivorship bias (dzisiejsze universe) i w UI jest oddzielona od loga realnego czerwonym `st.error` — dane symulowane i realne nigdy nie trafiaja do jednej liczby. `RULE_VERSION` w kazdym snapshocie; UI ostrzega przy mieszanych wersjach. **Benchmark GPW = `WIG20` przez `download_stooq()`** (yfinance zwraca 1 wiersz dla indeksow WIG); brak benchmarku degraduje do symulacji bez porownania, nie kasuje calej sekcji. Automatyzacja: `.github/workflows/top-picks.yml`, cron `0 6 1 * *`, `permissions: contents: write`. Skrypt konczy sie `sys.exit(1)` przy pokryciu danych <80%, cenach starszych niz 5 dni, niepelnej piatce **lub grupie `"?"`** — pusty/czesciowy snapshot klamalby w sekcji "Wyniki live" na zawsze. Lokalnie wymaga `GEM_CA_BUNDLE` (Norton robi TLS interception → `CERTIFICATE_VERIFY_FAILED` maskowane przez yfinance jako `TypeError: 'NoneType' object is not subscriptable`). **Przy okazji naprawione:** 5 tickerow z `SP500_TOP100` (BX/MMC/UBER/ANET/INTC) nie mialo wpisu w `SP500_SECTOR_MAP` — `ALL_SP500_TICKERS` 456 → 461. Spec: `docs/superpowers/specs/2026-08-06-top-picks-design.md`. Plan: `docs/superpowers/plans/2026-08-06-top-picks.md`. Tests: `tests/test_top_picks.py` 13/13 (pakiet 66/66).
  - **UI (2026-08-07):** strona przeniesiona z 24 na 9 (pod GPW) i przebudowana wizualnie. Piatka renderuje sie jako `top_pick_cards()` w `cards.py` — 5 kart z badge `#1…#5`, tickerem w zlocie, pillem sektora, zwrotem od rebalansu i **sparkline 3M jako inline SVG wewnatrz karty** (`_sparkline_svg()`, nie osobny `st.plotly_chart` — Plotly w kolumnie odklejalby sie od karty). Pod kartami `picks_return_bar()` (poziomy bar zwrotow) + `category_pie()` z licznikiem sektorow (widac dzialanie limitu 2/sektor); tabela fundamentalna zjechala do expandera. Metryki w "Wyniki live" i w symulacji to `kpi_card()` zamiast `st.metric` — doszla kolumna **vs benchmark w p.p.** i wartosc portfela. Naglowki rynkow: `section_band()`. Karta bez danych cenowych degraduje sie cicho (brak sparkline, zwrot `—`) — lokalnie bez `GEM_CA_BUNDLE` to normalny widok.
